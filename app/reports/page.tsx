...
function ReportsContent() {
  ...
  const frameworks: { nameKey: TranslationKey; descKey: TranslationKey; icon: string; color: string }[] = [
    { nameKey: 'report.fw.gri.name', descKey: 'report.fw.gri.desc', icon: '🌍', color: '#22C55E' },
    { nameKey: 'report.fw.csrd.name', descKey: 'report.fw.csrd.desc', icon: '🇪🇺', color: '#60A5FA' },
    { nameKey: 'report.fw.sdg.name', descKey: 'report.fw.sdg.desc', icon: '🎯', color: '#F59E0B' },
    { nameKey: 'report.fw.efrag.name', descKey: 'report.fw.efrag.desc', icon: '📐', color: '#8B5CF6' },
  ];

  return (
    <div style={{ padding: '32px 24px', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 28, color: 'var(--text-primary)', marginBottom: 4 }}>{t('report.title')}</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>{t('report.subtitle')}</p>
      </div>

      {/* Frameworks */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
        {frameworks.map(fw => (
          <div key={fw.nameKey} style={{ background: 'var(--bg-surface)', borderRadius: 12, padding: 20, border: '1px solid var(--border)', textAlign: 'center' }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>{fw.icon}</div>
            <div style={{ fontFamily: 'var(--font-serif)', fontSize: 15, color: 'var(--text-primary)', marginBottom: 4 }}>{t(fw.nameKey)}</div>
      ...
      {/* Project Reports */}
      <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 20, color: 'var(--text-primary)', marginBottom: 16 }}>{t('report.projectReports')}</h2>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, background: 'var(--bg-surface)', borderRadius: 16, border: '1px solid var(--border)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-serif)', fontSize: 20, color: 'var(--text-primary)', marginBottom: 8 }}>{t('report.noReports')}</div>
          <div style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 24 }}>{t('report.noReportsDesc')}</div>
            ...
            return (
              <div key={project.id} style={{ background: 'var(--bg-surface)', borderRadius: 12, padding: 24, border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  ...
                  >
                    {t('report.viewProject')}
                  </button>
                </div>
                <div style={{ display: 'flex', gap: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#22C55E' }} />
                    <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{onTrack} {t('report.onTrack')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#F59E0B' }} />
                    <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{atRisk} {t('report.atRisk')}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#EF4444' }} />
                    <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{behind} {t('report.behind')}</span>
                  </div>
                  <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{indicators.length} {t('report.totalIndicators')}</span>
                </div>
              </div>
            );
          })}
