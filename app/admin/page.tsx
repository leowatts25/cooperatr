  ...
  return (
    <div style={{ padding: '32px 24px', maxWidth: 900, margin: '0 auto' }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 28, color: 'var(--text-primary)', marginBottom: 4 }}>Admin Dashboard</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>Manage user access to Cooperatr</p>
        {pendingCount > 0 && (
          <div style={{ marginTop: 12, padding: '8px 16px', background: '#F59E0B15', border: '1px solid #F59E0B33', borderRadius: 8, display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 13, color: '#F59E0B', fontWeight: 600 }}>{pendingCount} pending approval{pendingCount > 1 ? 's' : ''}</span>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, background: 'var(--bg-surface)', borderRadius: 16, border: '1px solid var(--border)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-serif)', fontSize: 20, color: 'var(--text-primary)', marginBottom: 8 }}>No users {filter !== 'all' ? `with status "${filter}"` : 'yet'}</div>
        </div>
      ) : (
            ...
            <div key={user.id} style={{
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {user.approved_at && (
                    <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      Approved {new Date(user.approved_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                    </span>
                  )}
                </div>
              </div>
